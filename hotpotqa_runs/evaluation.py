...
class StableDiffusionEval_test:
    ...
    def evaluateGeneratedImage(self, image_og, image_generated):
        ...
        image_og_preprocess = self.preprocess(Image.open(image_og)).unsqueeze(0).to(self.device)
        image_og_features = self.model.encode_image( image_og_preprocess)

        image_generated_preprocess = self.preprocess(Image.open(image_generated)).unsqueeze(0).to(self.device)
